...
- 2026-04-08 업데이트
- 브랜치: `main`
- 파이프라인 등록분 Qoo10에서 수동 삭제 완료
- 시트 초기화 + 기존 운영 상품 역수입 작업 예정
- 전략 고도화 방향 결정: 데이터 수집 기반 구축부터 시작

---

## 🔴 즉시 진행 — 운영 초기화 + 역수입

### [초기화-1] coupang_datas 시트 리셋
- 지침 파일: `oc-import-existing-goods.md`
- 스크립트: `backend/scripts/reset-coupang-datas.js` (신규 작성)
- 헤더 행 유지, 2행부터 전체 삭제
- `npm run sheets:reset:dry` → 확인 → `npm run sheets:reset`

### [초기화-2] Qoo10 기존 운영 상품 → coupang_datas 역수입
- 지침 파일: `oc-import-existing-goods.md`
- 스크립트: `scripts/qoo10-import-existing-goods.js` (신규 작성)
- `GetAllGoodsInfo` 전체 조회 → PK 결정 → 시트 upsert
- PK 규칙:
  - SellerCode가 숫자로만 구성 → `vendorItemId = SellerCode` (쿠팡 연결)
  - 그 외 → `vendorItemId = "EXT_" + qoo10ItemId` (비쿠팡 가상키)
- 채우는 필드: `vendorItemId`, `qoo10ItemId`, `ItemTitle`, `qoo10SellingPrice`, `qoo10SellerCode`, `status=LIVE`, `registrationMessage=[imported=qoo10_native]`, `updatedAt`
- 이미지 필드(StandardImage, ExtraImages, DetailImages)는 빈값으로 둠
- EXT_ 상품은 IMAGE/DESC changeFlag 자동 skip (`qoo10-auto-register.js` 수정 포함)
- `npm run qoo10:import:existing:dry` → 확인 → real 실행
...
- **운영 초기화 + 역수입 설계** 확정:
  - EXT_ 가상키 규칙 (SellerCode 非숫자 → EXT_{qoo10ItemId})
  - 이미지 필드는 빈값 유지, EXT_ IMAGE/DESC changeFlag skip 규칙 적용
  - 지침 파일: `oc-import-existing-goods.md`

### 2026-04-02
- **changeFlags 플래그별 테스트 완료** (PRICE/IMAGE/CATEGORY/TITLE/DESC/REBUILD/REFRESH 전체)
- **DetailImages 수집 버그 수정** — `buildImageExtractFn()` return `detail` 누락 수정 (oc/fix-detail-images)
...
- **PRICE 플래그 qty 버그 수정** — `qty: 100` → `qty: null`
- **CATEGORY 플래그 jpTitle write-back 버그 수정**
