...
| 2. 상세 수집 | `npm run coupang:collect` | ❌ 생략 | Browser Relay attach 상태에서만 실행 — dry-run도 실제 fetch 발생 |
...
# 2. Playwright 브라우저 데몬 (stock:check 사용 시에만 필요)
npm run coupang:browser:start

# 3. 쿠키 갱신 (만료 시에만, 유효하면 자동 skip)
npm run cookie:refresh
```

> **수집(coupang:collect) 전 필수:** Chrome에서 쿠팡 로그인 탭 열고 Browser Relay 확장 아이콘 클릭 → badge ON.
> 이 attach 작업은 하루 1회만 하면 된다. attach된 동안 수집 스크립트가 자율 실행된다.
>
> **주의:** `coupang:browser:start`는 `coupang:collect`에 더 이상 필요하지 않다.
> `coupang-stock-monitor.js`가 Playwright를 사용하므로 stock:check 실행 시에만 필요.
...
### Browser Relay attach 안 된 상태에서 coupang:collect 실행

**증상:** 수집 시작 즉시 전 상품 NAVIGATE_ERROR 또는 evaluate 타임아웃

**원인:** Chrome 쿠팡 탭에 Browser Relay가 attach되지 않은 상태

**해결:**
1. Chrome에서 쿠팡 로그인 탭 확인
2. Browser Relay 확장 아이콘 클릭 → badge ON 확인
3. `npm run coupang:collect` 재실행
